...
この人がどういう人なのかを、人物描写として1〜3文で書く。
「今回の分析から〜が分かった」のような報告調は禁止。
「○○を大切にしている」「○○に不安を感じやすい」「○○という経験が価値観の土台になっている」のように、その人自身を主語にして書く。
前回のポートレートがあればそれを踏まえて上書き・補完する形で。
新たな発見がなければ空文字列にする。
...
export async function analyzePersonCentric(
  ...
  if (parsed.supported === true) {
    return {
      ...
      avoidWords: [],
      sources: [],
      recommendedBooks: [],  portraitUpdate: "",
    };
  }
